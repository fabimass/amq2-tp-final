...
Este DAG vuelve a entrenar el modelo basándose en nuevos datos, prueba el modelo anterior y pone en producción el nuevo 
si funciona mejor que el anterior. Utiliza el Mean Absolute Error para evaluar el modelo con los datos de prueba.
...
def processing_dag():
    ...
    def train_the_challenger_model():
        ...
        def register_challenger(model, mae, model_uri):
        ...
        # Record the model
        register_challenger(challenger_model, mae, artifact_uri)


    @task.virtualenv(
        task_id="challenge_the_champion",
        requirements=["scikit-learn==1.3.2",
                      "mlflow==2.10.2",
                      "awswrangler==3.6.0",
                      "xgboost"],
    ...
    def evaluate_champion_challenge():
        ...
        with mlflow.start_run(run_id=list_run[0].info.run_id):
            mlflow.log_metric("test_f1_challenger", mae_challenger)
            mlflow.log_metric("test_f1_champion", mae_champion)
